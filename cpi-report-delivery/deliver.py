r"""Generate a PDF report, upload PDF and CSV to S3, and send email via GL Connectors.

This script is invoked by the cpi_reporting_agent via the ``execute`` filesystem tool.
It reads a pre-generated markdown report file and the anomaly CSV written by the
``cpi-anomaly-csv-exporter`` skill, then:
  ...
  2. Uploads both the PDF and CSV to S3 using urllib + AWS SigV4 signing (no boto3).
  3. Authenticates with GL Connectors and sends an email with both files attached,
     using urllib with multipart/form-data encoding (no requests).

All credentials are read from environment variables injected into the agent sandbox.
Runtime requirements: ``weasyprint`` (Python) and the Pango/Cairo system libraries.
...
Usage:
    python deliver.py \\
        ...
        --email-subject "Laporan CPI Maret 2026" \\
        [--email-body-file /workspace/tmp/email_body.md] \\
        [--s3-bucket glair-cpi-staging] \\
        [--s3-prefix cpi_report_base] \\
        [--skip-s3] \\
        [--skip-email]
...
Environment variables (injected by agent sandbox_env):
    ...
    CPI_S3_BUCKET, CPI_S3_PREFIX (fallbacks when --s3-bucket/--s3-prefix not given)

On success prints a single JSON line to stdout:
    {"pdf_path": "...", "csv_path": "...", "s3_uris": [...], "email_sent": true}
...
def _parse_flag_token(token: str, it: Any, args: dict[str, Any]) -> bool:
    ...
    Args:
        token: The current CLI flag string (e.g. ``--csv-path``).
...
def _parse_args(argv: list[str]) -> dict[str, Any]:
    ...
    Returns:
        dict[str, Any]: Parsed configuration with keys: csv_path, markdown_file,
    ...
    args: dict[str, Any] = {
        ...
        "email_to": [],
        "email_subject": "",
        "s3_bucket": os.environ.get("CPI_S3_BUCKET", ""),
        "s3_prefix": os.environ.get("CPI_S3_PREFIX", "cpi_report_base"),
        "skip_s3": False,
        "skip_email": False,
...
def _generate_pdf(markdown_file: str, output_dir: str) -> str:
    ...
    pdf_path = os.path.join(output_dir, "report_cpi.pdf")
...
def _upload_to_s3(file_paths: list[str], s3_bucket: str, s3_prefix: str) -> list[str]:
    ...
    boto3's ``upload_file`` transparently uses multipart upload for large
    files (threshold: 8 MiB by default) and streams data in chunks, so the
    entire file is never loaded into RAM at once.
...
def _send_email(  # noqa: PLR0913
    ...
    # Build form fields — mail_to repeats for each recipient.
...
def _handle_s3_upload(args: dict[str, Any], pdf_path: str) -> list[str]:
    """Upload PDF and CSV to S3 if configured."""
    if args["skip_s3"] or not args["s3_bucket"]:
        return []
    
    try:
        return _upload_to_s3(
            [pdf_path, args["csv_path"]],
            args["s3_bucket"],
            args["s3_prefix"],
        )
    except Exception as exc:
...
def _handle_email(args: dict[str, Any], pdf_path: str, output_dir: str) -> bool:
    """Send an email with the PDF and CSV attachments."""
...
def main(argv: list[str]) -> None:
    ...
    # Handle append-markdown action
    if args.get("append_markdown") is not None:
        try:
            with open(args["markdown_file"], "a", encoding="utf-8") as fh:
                fh.write(args["append_markdown"] + "\n")
            sys.stdout.write(json.dumps({"success": True, "action": "append_markdown", "file": args["markdown_file"]}) + "\n")
            return
        except OSError as exc:
    ...
    if not os.path.isfile(args["markdown_file"]) or os.path.getsize(args["markdown_file"]) == 0:
        _fail(f"Validation failed: markdown file {args['markdown_file']!r} is missing or empty. Please generate the report first.")
    
    if not os.path.isfile(args["csv_path"]) or os.path.getsize(args["csv_path"]) == 0:
        _fail(f"Validation failed: CSV file {args['csv_path']!r} is missing or empty. Please ensure anomaly export was successful.")

    # Step 1: Generate PDF from markdown.
    # Use tempfile.mkdtemp() so the output directory is created with mode 0o700
    # (owner-only), preventing symlink attacks against the world-writable /tmp.
    output_dir = tempfile.mkdtemp(prefix="cpi_report_")
    pdf_path = _generate_pdf(args["markdown_file"], output_dir)

    # Step 2: Upload PDF + CSV to S3 (non-fatal — failure only affects archival).
    s3_uris = _handle_s3_upload(args, pdf_path)

    # Step 3: Send email with both files attached.
